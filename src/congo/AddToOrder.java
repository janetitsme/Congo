...
// Allows the user to buy items.It checks if the user is logged. If yes only then he is allowed to purchase
package congo;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if (cookieList != null) 
			{
			    for(int i=0; i<cookieList.length; i++) 
			    {
			        ...
			        //String userTest=cookie.getName();
			        if(cookie.getName().equals("user") )
			        {
			        if (!rconnectLogin.checkUserId(cookie.getValue())) 
			        {
			        	RequestDispatcher dispatcher= request.getRequestDispatcher("/login.html");
			        	request.setAttribute("login","Please login to purchase items");
			        ...
			        else
			        {
						...
						CongoBean c = congoData.AddToOrder(artistname);	//called the AddToOrder method from DAO
						  	...
						  	out.println("<p>The following album is added to your order</p><br>");
